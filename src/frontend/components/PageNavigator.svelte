...
<nav class="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
  <button
    class="relative inline-flex items-center px-1 md:px-2 py-1 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-100"
  ...
  {#each range(minPageNum, maxPageNum + 1, 1) as i}
    <button
      class={`relative inline-flex items-center font-medium text-sm px-3 md:px-4 py-1 border ${i === currentPage ? 'text-white border-indigo-400 bg-indigo-700' : 'text-gray-500 border-gray-300 bg-white hover:bg-gray-100'}`}
      on:click={() => onPageChanged(i)}>{i}</button>
  {/each}
  <button
    class="relative inline-flex items-center px-1 md:px-2 py-1 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-100"
